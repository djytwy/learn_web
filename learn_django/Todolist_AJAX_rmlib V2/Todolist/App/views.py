...
def todolist(request):
    ...
    if from_source == 'IE8':
        ...
        for i in things_list:
            time_se = (time - i.time).seconds
            day_sub = (time - i.time).days
            if day_sub >= 1:
                hour = time_se // 3600
                minutes = (time_se % 3600)//60
                sec = time_se % 60
                time_sub = '%s天 - %s时 - %s分前' % (day_sub, hour, minutes)
            elif time_se >= 3600:
                hour = time_se // 3600
                minutes = (time_se % 3600) // 60
                sec = time_se % 60
                time_sub = '%s时 - %s分前' % (hour, minutes)
            elif 60 < time_se < 3600:
                minutes = time_se // 60
                time_sub = '%s分前' % minutes
            else:
                time_sub = '%s秒前' % time_se
            Things.objects.filter(id=i.id).update(time_sub=time_sub)
        return render(request, 'index.html', locals())
...
def add_things(request):
    ...
    if Things.objects.filter(things=thing).first() is None:
        Things.objects.create(things=thing)
        data_temp = Things.objects.filter(things=thing).values() #Things.objects.filter(things=thing).values()返回类型是一个列表，内容的格式为字典[{},{}]
        num_temp = data_temp[0]
        num = num_temp['id']
        Things.objects.filter(things=thing).update(time_sub='1秒前')
        data = "<div class='row' id='%d'>" \
               "<div class='col-md-10'>" \
               "<div class='form-group'>" \
               "<textarea class='form-control ' rows='3' name='text' id=things_text%d>" \
               "%s --- 1秒前" \
               "</textarea>" \
               "</div>" \
               "</div>" \
               ...
               "<input type='submit' class='btn btn-warning' value='Delete' name='Delete' id=%d>" \
               "<input type='submit' class='btn btn-success' value='Edit' name='Edit' id=%d>" \
               "</div>" \
               "<div class='checkbox' id='%d'>" \
               "<label><input type='checkbox'>已经做了</label>" \
               "</div></div></div>" % (num, num, thing, num, num, num, )
...
def Edit_thing(request):
    if request.method == 'POST':
        ...
        Things.objects.filter(id=things_id).update(time=time)
        thing_update = thing_update + ' --- 1秒前'
